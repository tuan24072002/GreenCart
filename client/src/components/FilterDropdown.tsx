...
const FilterDropdown = ({ className, category }: Props) => {
  ...
  const handlePriceBlur = (type: "min" | "max") => {
    if (type === "min" && priceRange.min) {
      ...
      if (!isNaN(numberValue)) {
        setPriceRange({
          ...priceRange,
          min: numberValue.toLocaleString() + " VNĐ",
        });
      }
      ...
      if (!isNaN(numberValue)) {
        setPriceRange({
          ...priceRange,
          max: numberValue.toLocaleString() + " VNĐ",
        });
      }
    } else {
      return;
  ...
  const handlePriceFocus = (type: "min" | "max") => {
    if (type === "min" && priceRange.min) {
      const numericValue = priceRange.min
        .replace(/,/g, "")
        .replace(/ VNĐ/g, "");
      setPriceRange({ ...priceRange, min: numericValue });
    } else if (type === "max" && priceRange.max) {
      const numericValue = priceRange.max
        .replace(/,/g, "")
        .replace(/ VNĐ/g, "");
    ...
    <div className={cn("", className)}>
      <div className="w-full">
        ...
        <select
          ...
          className="w-full p-2 border rounded-md focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary bg-background"
        >
